  ...
  return (
    <div className="flex min-h-screen flex-col">
      ...
      <main id="main-content" className="flex flex-1 items-center justify-center p-4">
        <ErrorState
          title={isNotFound ? 'Scenario not found' : 'Failed to load scenario'}
          message={
            isNotFound
              ? 'The scenario you are looking for does not exist or has been removed.'
              : isNetworkError
                ? 'Unable to connect to the server. Please check your connection and try again.'
                : error.message ||
                  'An error occurred while loading this scenario. Please try again.'
          }
          variant={isNotFound ? 'not-found' : isNetworkError ? 'network' : 'default'}
          onRetry={isNotFound ? undefined : reset}
          backHref="/scenarios"
          backLabel="Browse all scenarios"
        />
      </main>
